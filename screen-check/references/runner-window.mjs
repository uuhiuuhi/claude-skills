...
//   ① ~/.baroos-auto/runner.lock 존재(러너 배치 실행 중)
...
export function runnerBusy() {
  const reasons = []
  if (existsSync(resolve(STATE, 'runner.lock'))) reasons.push('runner.lock 존재')
  ...
  try {
    const out = execSync('powershell -NoProfile -Command "Get-CimInstance Win32_Process -Filter \\"Name=\'node.exe\'\\" | ForEach-Object { $_.CommandLine }"', { encoding: 'utf8', timeout: 15000 })
    const hits = out.split(/\r?\n/).filter((l) => /vitest|tsc\b|eslint|run-slots|night/.test(l) && !/screen-check|e2e-tools/.test(l))
    if (hits.length) reasons.push(`qa 프로세스 ${hits.length}개(vitest/tsc/eslint/러너)`)
