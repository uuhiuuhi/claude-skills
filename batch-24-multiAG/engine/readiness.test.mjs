...
const okManifest = (over = {}) => ({
  schema: 'batch-24-multiag/verification/1',
  ...
  completion: {
    ...
    verdict: READY, counts: { pass: 9, fail: 0, notVerified: 0, total: 9 }, notVerified: [],
  },
  ...over,
})
...
const okDiagnosis = (over = {}) => ({
  schema: 'batch-24-multiag/diagnosis/1', at: '2026-09-03T00:00:00.000Z',
...
describe('readiness — 기준표 형태', () => {
  it('작업 8조건·프로젝트 8조건이 T1~T8 / P1~P8 로 빠짐없이 있고, 각 조건이 근거 출처를 들고 있다', () => {
    assert.deepEqual(TASK_CRITERIA.map((c) => c.id), ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'Q9'])
...
describe('readiness — 작업(스토리) 8조건', () => {
  it('여덟 조건이 전부 통과하면 ready 다(기준선)', () => {
    ...
    assert.equal(r.counts.pass, 9)
    assert.equal(r.schema, 'batch-24-multiag/readiness/1')
  })
  ...
  it('fail 1건 + not-verified 2건이면 not-verified 가 아니라 not-ready 다(더 나쁜 쪽이 이긴다)', () => {
    ...
    assert.equal(r.counts.notVerified, 4)
...
describe('readiness — 판정표 렌더', () => {
  it('여덟 줄 + 판정 + 근거가 나오고, 확인 못 한 것을 「통과」로 적지 않는다', () => {
    ...
    for (const id of ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'Q9']) assert.ok(md.includes(`| ${id} |`), `${id} 줄 없음`)
...
describe('readiness — 프로젝트 8조건(실제 픽스처 진단)', () => {
  ...
  it('전부 깨끗하면(지적 0 · 게이트 GREEN · 교차 리뷰 있음) ready 가 나온다', () => {
    const clean = {
      schema: 'batch-24-multiag/diagnosis/1', at: '2026-09-03T00:00:00.000Z', root: '가짜',
