...
export function asfCandidates(relName, { here = HERE, home = homedir() } = {}) {
  ...
  // One pinned runtime for both the executable and its helper modules.
  if (process.env.AUTO_STORY_RUNTIME) return [resolve(process.env.AUTO_STORY_RUNTIME, name)]
  const pinned = resolve(here, 'runtime')
  if (existsSync(pinned)) return [join(pinned, name)]
