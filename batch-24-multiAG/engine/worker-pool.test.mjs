...
describe('[workers] 설정 정규화 — 없으면 종전 동작(configured=false · Claude 전용 · 하드캡 3)', () => {
  ...
  it('[#7] 폐지된 pushOnFail 은 정규화 결과에 남지 않고 경고만 남는다(조용히 먹지 않는다)', () => {
    ...
    // First RED must stop: legacy retry values are ignored and positive requests are reported.
    assert.equal(p.integrationGate.retry, 0)
    assert.equal(providerConfig({ integrationGate: { enabled: true, retry: 5 } }).integrationGate.retry, 0)
    assert.equal(providerConfig({ integrationGate: { enabled: true, retry: 0 } }).integrationGate.retry, 0)
    assert.equal(providerConfig({ integrationGate: { enabled: true, retry: 'x' } }).integrationGate.retry, 0)
...
describe('[workers] 통합 게이트 판정 · 엔진 플래그 변환 · exit 정보', () => {
  ...
  it('[#17] applyIntegrationToManifest — pass/fail/rollback 병합 · 원본 불변 · 알 수 없는 값은 fail 로 닫는다', () => {
    const base = { schema: 'batch-24-multiag/verification/1', checks: { qa: 'pass' } }
    ...
    assert.deepEqual(pass.checks, { qa: 'pass', integration: 'pass' }, '기존 필드 보존')
...
describe('[run-night] 배선 앵커 — 순수 규칙이 실제로 러너에 꽂혀 있다(배선이 죽으면 종전 동작으로 조용히 되돌아간다)', () => {
  ...
  it('워커 풀·위험·통합 게이트·엔진 플래그·exit 정보·증거 보관이 배선돼 있다', () => {
    ...
    for (const s of ['pickRunnable(pending, [...running.values()], caps)', 'parallelHazards(lists, { judges: [parallelHazardsCompat] })', 'integrationGateDecision({ enabled: true', "['reset', '--hard', landingBase]", 'engineFlagsFromConfig(PCFG)', 'blockedProviderFromExit(info)', 'archiveEvidence(wt)', 'parallelPlanWithWorkers({', 'applyIntegrationToManifest(']) assert.ok(src.includes(s), `누락: ${s}`)
  })
  ...
  it('[#16] 통합 결과가 스토리 매니페스트·배치 매니페스트로 간다 · 없는 매니페스트를 만들지 않는다', () => {
    ...
    assert.ok(src.includes('batch-24-multiag/batch-manifest/1'))
    ...
    assert.doesNotMatch(src, /\[INTEGRATION\]\[RETRY\]/, 'first RED must never retry into GREEN');
    assert.ok(providerConfig({ integrationGate: { retry: 1 } }).warnings.some(w => /retry/.test(w)), 'ignored legacy retry must be reported');
