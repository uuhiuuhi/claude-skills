...
import { spawnSafe } from './runtime/providers/spawn-safe.mjs'
 ...
 */
function makeE2E({ traps = {}, config = {}, scripts = {}, isolated = true } = {}) {
  ...
  for (const f of readdirSync(join(REPO, 'batch-24-multiAG', 'engine', 'runtime')).filter((n) => n.endsWith('.mjs') && !n.endsWith('.test.mjs'))) {
    cpSync(join(REPO, 'batch-24-multiAG', 'engine', 'runtime', f), join(skill, f))
  }
  ...
  if (isolated) for (const args of [['init', '-q', seed], ['-C', seed, '-c', 'user.name=e2e', '-c', 'user.email=e2e@test', 'commit', '--allow-empty', '-qm', 'seed'], ['-C', seed, 'worktree', 'add', '-q', '-b', 'main', proj, 'HEAD']]) {
    const r = spawnSync('git', args, { encoding: 'utf8' }); assert.equal(r.status, 0, r.stderr);
  }
...
function runAF(e, args = [], env = {}) {
  ...
  const details = outDir ? readdirSync(outDir).filter(n => /runner.*\.log$/.test(n)).map(n => readFileSync(join(outDir, n), 'utf8').split('\n').filter(l => /DETAIL|STOP/.test(l)).join('\n')).join('\n') : '';
...
describe('E2 · E11 — 1라운드 왕복과 게이트 예산', () => {
  it('라운드 2회를 돌고 · 큐가 검증기를 통과하고 · 전체 qa 는 landing 근거를 재사용하고 · build 는 1회 · 실제 dev/review 가 돈다', () => {
    const e = makeE2E()
    ...
    assert.equal(r.run.gateCalls.qa ?? 0, 0, 'landing에서 완료한 전체 회귀를 반복하면 안 된다')
    assert.equal(r.run.gateCalls.build, 1, 'build 는 마지막 1회뿐이다')
    assert.ok(qa >= (r.run.gateCalls.qa ?? 0), `실제 qa 실행 ${qa}회 — 러너의 스토리 qa 를 포함해 게이트 호출 이상이어야 한다`)
    assert.ok(build >= 1)
    ...
    assert.ok(readCodexCalls(e.stub).some(c => c.kind === 'exec'), '독립 Codex review 호출이 없다')
...
describe('E3 · E4 — Fable 계획 채택과 거부 폴백', () => {
  ...
  it('E4: 후보에 없는 스토리를 지어내면 계획 전체를 버리고 규칙 계획으로 계속 돈다(밤이 서지 않는다)', () => {
    ...
    assert.equal(r.status, 0, r.out)
    const p = r.readJson('round-0-plan.json')
    assert.match(p.source, /^deterministic-fallback\((?:contract:)?invented-story/, `출처 ${p.source}`)
...
// ═══════════════════════════════════════════════════════════════════════════
describe('E9 — 전체 QA는 최종 한 번 실행하고 RED를 보고·종료 코드에 반영한다', () => {
  ...
  it('라운드마다 전체 QA를 반복하지 않고 최종 RED이면 성공으로 끝내지 않는다', async () => {
    const e = makeE2E()
    const RED = 'FAIL  tests/feature/c.test.ts > 저장한다\nAssertionError: expected 1 to be 2\n'
    let round = 0, qaCalls = 0
    const r = await runAutoFinish({
      root: e.proj, state: e.afState, maxRounds: 5, gates: ['qa'], log: () => {}, planRunner: false,
      exec: () => { qaCalls++; return { status: 1, stdout: RED, stderr: '' }; },
      spawnRunner: () => {
    ...
    assert.equal(qaCalls, 1, '전체 QA는 최종 한 번이어야 한다');
    assert.equal(r.exitCode, 1, '최종 QA RED를 성공으로 반환했다');
    assert.ok(round > 0 && round <= 5, '설정된 라운드 예산을 벗어났다');
    assert.equal(r.diagnoses.at(-1).gates.qa.exit, 1);
...
describe('교차 리뷰 — 만든 쪽과 다른 제공자가 검토한다', () => {
  ...
  it('허용 스위치가 없으면 본 트리에서는 codex 및 자체 리뷰를 차단한다(반출 방지 가드)', () => {
    const e = makeE2E({ isolated: false, config: { providers: { codex: { enabled: true, max: 1, roles: ['review'] } } } })
    ...
    assert.match(log, /same-provider review forbidden|codex.*폴백/, '보호 조건의 중단 사유가 필요하다')
    assert.equal(readCalls(e.stub).filter(c => c.kind === 'review').length, 0)
  })
})
