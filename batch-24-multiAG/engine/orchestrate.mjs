...
export function buildPlanPrompt(context = {}) {
  ...
  const lines = [
    ...
    ...(full ? [
      ...
      '각 후보의 stages 는 규칙이 정한 것이다(replan = 시니어 재계획 · mockup = AI 목업 초안) — 바꾸지 말고 순서·짝·병렬 폭만 정한다. notes/replanHint 는 판단 재료다.',
    ...
    '- models 는 생략한다. 단계별 모델은 실행 직전에 품질 기준·가용성·공유 한도 상태로 결정한다.',
    '',
...
export async function requestPlan({ context = {}, dag = null, constraints = {}, deterministic = null, runner = null, schema = PLAN_SCHEMA, timeoutMs = null } = {}) {
  ...
  const contractErrors = validateCandidateContract(parsed.plan, context.candidates ?? [], { requireAll: context.mode === 'full' })
  if (contractErrors.length) return fallback('contract:' + contractErrors[0])
...
/** The planner may reorder work, but cannot remove work or change its required stages. */
export function validateCandidateContract(plan, candidates, { requireAll = false } = {}) {
  const errors = []
  const expected = new Map(candidates.map((c) => [String(c.key ?? c), c]))
  const seen = new Set()
  for (const batch of plan.batches ?? []) {
    const signatures = new Set()
    for (const key of batch.stories ?? []) {
      if (seen.has(key)) errors.push('duplicate-story')
      seen.add(key)
      const source = expected.get(key)
      if (!source) { errors.push('invented-story'); continue }
      if (Array.isArray(source.stages)) {
        const signature = JSON.stringify(source.stages)
        signatures.add(signature)
        if (batch.stages !== undefined && JSON.stringify(batch.stages) !== signature) errors.push('stage-mismatch')
      }
    }
    if (signatures.size > 1) errors.push('mixed-stages')
    if (batch.models?.dev && batch.models.dev === batch.models?.review) errors.push('self-review')
  }
  if (requireAll && [...expected.keys()].some((k) => !seen.has(k))) errors.push('missing-candidates')
