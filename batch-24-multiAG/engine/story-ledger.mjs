 ...
 */
export const isHumanGateLine = (line) => /사람 게이트|승인자|👤/.test(line)
...
export function parseSprint(text) {
  // Concurrent story updates can leave duplicate YAML keys. Treat the last row
  // as authoritative (the same effective value most YAML consumers expose) so
  // one story is never scheduled twice and then removed entirely by validation.
  const rows = new Map()
  for (const line of text.split('\n')) {
    const m = /^ {2}(\d+-\d+[^:]*): *(backlog|ready-for-dev|in-progress|review|done)\b/.exec(line)
    if (m) rows.set(m[1], { key: m[1], status: m[2], epic: Number(m[1].split('-')[0]) })
  }
  return [...rows.values()]
}
