...
// Fable 계획 캐시를 기본 활성화해 반복 슬롯의 불필요한 호출을 줄인다.
...
function readWorktreeTimeline(wt) {
  ...
  let stages = []
  try { stages = parseEngineLog(readFileSync(join(logs, 'run-summary.log'), 'utf8'), { story: wt.story })
    .filter((e) => Date.parse(e.start) >= Date.parse(wt.metrics?.start) - 1000 && Date.parse(e.start) <= Date.parse(wt.metrics?.end)) } catch { stages = [] }
  const tokens = {}
  try {
    for (const f of readdirSync(logs).filter((n) => n.startsWith(wt.story) && n.endsWith('.log'))) {
      if (!stages.length) continue
      if (statSync(join(logs, f)).mtimeMs < Date.parse(wt.metrics?.start)) continue
...
async function applyOrchestrator(q, outPath) {
  ...
  const adopt = (plan, source, cacheState) => {
    ...
    q.batches = plan.batches.map((b, i) => {
      ...
      return {
        ...
        enabled: true,
        stories: b.stories ?? [],
        stages: src.stages ?? ['dev'],
        ...(src.force ? { force: true } : {}),
        ...(src.models ? { models: src.models } : {}),
  ...
  const fingerprint = orchFingerprint({
    ...
    models: { codex: PCFG.providers.codex.enabled ? await codexAvailable() : false,
      policy: CFG.modelPolicy, exhausted: CFG.exhaustedModels, providers: CFG.providers },
  })
  ...
  if (cache?.plan && cache.fingerprint === fingerprint && Number.isFinite(ageH) && ageH >= 0 && ageH <= ORCH.cacheHours) {
    ...
    if (hit.source === 'fable') { PLAN_MODEL = cache.model ?? ORCH.model; adopt(hit.plan, `${PLAN_MODEL}(cache)`, 'cache hit'); return }
  ...
  if (!CFG.modelPolicy?.enabled && cache?.cooldownUntil && Date.parse(cache.cooldownUntil) > Date.now()) {
  ...
  const noteRunnerError = () => {
    ...
    if (errors >= ORCH_COOLDOWN_AFTER && !CFG.modelPolicy?.enabled) next.cooldownUntil = new Date(Date.now() + ORCH.cacheHours * 3_600_000).toISOString()
    writeOrchCache(next)
  }
  ...
  for (let i = 0; i < ladder.length; i++) {
    ...
    if (CFG.modelPolicy?.enabled && readModelHealth(STATE_DIR).blocked(model)) continue
    let runner
    ...
    if (CFG.modelPolicy?.enabled && !process.env.AUTO_PLAN_RUNNER_STUB) {
      const kind = shouldLadderOn(res.source) ? failureKind(res.plan?.errorDetail ?? res.source) : 'success'
      recordModelEvent(STATE_DIR, { model, kind, role: 'orchestrate', startedAt: requestStarted, detail: res.plan?.errorDetail ?? '' })
    }
    usedModel = model
    PLAN_MODEL = model
  ...
  if (res.source === 'fable') {
    if (!adopt(res.plan, usedModel, 'cache miss')) return
...
async function runBatchParallel({ batch, defaults, workers, record }) {
  ...
  const reassign = (list) => {
    const assigned = assignWorkers({
      ...
      history,
      config: { models: batch.models ?? {}, split: Boolean(PCFG.providers.codex.split), modelPolicy: CFG.modelPolicy,
        exhaustedModels: CFG.exhaustedModels, blocked: readModelHealth(STATE_DIR).blocked },
    })
    for (const a of assigned) {
      ...
      if (!CFG.modelPolicy?.enabled && a.risk < 4 && blocked.includes('claude') && codexOk && (PCFG.providers.codex.roles ?? []).includes('dev') && specProvider(dev) === 'claude') {
        dev = 'codex'
