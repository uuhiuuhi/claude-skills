...
// ═══════════════════════════════════════════════════════════════════════════
describe('게이트 예산 — 전체 검사 최종 1회', () => {
  it('qa 는 최종 1회 · build 는 마지막 1회뿐이다', async () => {
    const fx = project()
    const state = tmp('af-state-')
    const seen = []
    ...
    const r = await runAutoFinish({
      ...
      exec, spawnRunner: () => { fx.write('src/final-check.ts', 'export const changed = 1\n'); return { status: 0, stdout: '', stderr: '' }; }, planRunner: false, bmadWrites: 'plan',
    })
    const qa = seen.filter((s) => s === 'qa').length
    const build = seen.filter((s) => s === 'build').length
    assert.equal(qa, 1, '최종 qa는 한 번만 실행한다')
  ...
  it('--no-gates 는 게이트를 한 번도 돌리지 않는다', async () => {
    ...
    const r = await runAutoFinish({
      ...
      exec: () => { calls++; return { status: 0 } }, spawnRunner: () => { fx.write('src/final-check.ts', 'export const changed = 1\n'); return { status: 0, stdout: '', stderr: '' }; }, planRunner: false,
    })
    assert.equal(calls, 0)
    assert.deepEqual(r.gateCalls, {})
  })
...
describe('예산 deadline — spawn timeout 을 잔여 시간으로 자르고, 다 쓰면 최종 게이트도 건너뛴다', () => {
  ...
  it('--budget-min 이 짧고 스텁이 느리면 러너 timeout 이 잔여 시간으로 잘리고 · 예산 소진이 사유로 남는다', async () => {
    const fx = project()
    const state = tmp('af-state-')
    const budgetMin = 0.05; let clockMs = Date.parse('2026-09-06T00:00:00Z'); const initialMs = clockMs;
    let runnerTimeout = null
    const r = await runAutoFinish({
      root: fx.root, state, now: () => new Date(clockMs++), maxRounds: 1, budgetMin, gates: ['qa'], log: () => {}, planRunner: false, dryRun: true,
      exec: () => { sleep(1500); return { status: 0, stdout: '', stderr: '' } },
      spawnRunner: (_bin, _args, opts) => { runnerTimeout = opts.timeout; clockMs = initialMs + 4000; return { status: 0, stdout: '', stderr: '' } },
    })
    ...
    assert.equal(r.gateCalls.qa ?? 0, 0, `qa ${r.gateCalls.qa}회 — 예산이 없으면 최종 게이트는 건너뛴다`)
  ...
  it('예산이 다하면 BMAD 등재(대상 저장소 쓰기)를 하지 않는다 — 파일 지문이 그대로다', async () => {
    ...
    let runnerCalls = 0; const start = Date.parse('2026-09-06T00:00:00Z');
    const r = await runAutoFinish({
      root: fx.root, state, runId: 'deadline-bmad', now: () => new Date(start + (existsSync(join(state, 'autofinish/deadline-bmad/round-0-bmad-plan.json')) ? 3000 : 0)), maxRounds: 2, budgetMin: 0.04, gates: ['qa'], log: () => {},
  ...
  it('예산이 넉넉하면 종전대로 최종 게이트까지 돈다(회귀 없음)', async () => {
    ...
    assert.equal(r.gateCalls.qa ?? 0, 0, 'dry plan에 코드 변경이 없으면 qa를 실행하지 않는다')
