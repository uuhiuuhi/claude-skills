...
const findings = process.env.E2E_CODEX_FINDING === '1' && /2-1-a/.test(story) && diff.includes('src/a.ts')
...
const reviewedFiles = [diffFile, ...[...diff.matchAll(/^diff --git a\/(.+?) b\/(.+)$/gm)].map(m => m[2])];
for (const file of reviewedFiles) { try { readFileSync(join(process.cwd(), file), 'utf8'); ev({ type: 'item.completed', item: { id: file, type: 'command_execution', command: 'cat ' + JSON.stringify(file), exit_code: 0 } }); } catch {} }
...
function makeFixture({ config = {}, models = { dev: 'fable', review: 'codex' }, queueDefaults = {}, stages = ['dev', 'review'], storyStatus = 'ready-for-dev', storyFiles = { '2-1-a': 'src/a.ts', '2-2-b': 'src/b.ts' } } = {}) {
  ...
  for (const f of readdirSync(join(REPO, 'batch-24-multiAG', 'engine', 'runtime')).filter((n) => n.endsWith('.mjs') && !n.endsWith('.test.mjs'))) cpSync(join(REPO, 'batch-24-multiAG', 'engine', 'runtime', f), join(skill, f))
  cpSync(join(REPO, 'batch-24-multiAG', 'engine', 'runtime', 'providers'), join(skill, 'providers'), { recursive: true })
  // 스텁 실행 파일
  ...
  ok(git(T, ['clone', '-q', origin, seed]), 'clone');
  ok(git(seed, ['-c', 'user.name=e2e', '-c', 'user.email=e2e@test', 'commit', '--allow-empty', '-qm', 'seed']), 'seed');
  ok(git(seed, ['worktree', 'add', '-q', '-b', 'main', proj, 'HEAD']), 'linked project');
...
const fixtures = []
after(() => { if (process.env.KEEP_E2E_TMP === '1') { console.log(fixtures.map(f => f.T)); return; } for (const fx of fixtures) { try { rmSync(fx.T, { recursive: true, force: true }) } catch { /* 잠긴 파일은 OS 가 정리 */ } } })
...
describe('[e2e] 병렬 워커 풀 → 엔진(claude dev · codex review) → landing → 통합 게이트', { timeout: 300_000 }, () => {
  ...
  it('엔진: 프로바이더 감지 줄 · codex 리뷰가 원장 형식으로 기재 · 상태 전이(findings → in-progress · clean → done) · sprint-status 동기', () => {
    ...
    assert.match(a, /Clean review/)
    assert.match(a, /^Status: done/m)
    ...
    assert.match(sprint, /^  2-1-a: done/m)
    assert.match(sprint, /^  2-2-b: done/m)
    assert.match(r.runLog, /\[2-1-a\]\[CODEX\]\[REVIEW\] 기재 완료 — decision 0 · patch 0\(high 0\)/)
    // 현황판이 읽는 줄 형식 그대로
  ...
  it('검증 매니페스트 · .env 격리 후 복원 · transient 파일 0 · 워크트리 정리 · 시크릿이 로그에 없다', () => {
    ...
    assert.equal(m.schema, 'batch-24-multiag/verification/1')
    ...
    assert.equal(m.review.counts.patch, 0)
...
const gateFixture = () => makeFixture({
  ...
  config: { providers: { codex: { enabled: true } }, integrationGate: { enabled: true, pushOnFail: true, retry: 1 } },
...
describe('[e2e][#7] 통합 RED — pushOnFail 이 켜져 있어도 원격에 아무것도 올라가지 않는다', { timeout: 300_000 }, () => {
  ...
  it('폐지 경고 · rollback · 원격 ref 0건 · 로컬 landing 0건 · exit 1', () => {
    ...
    const calls = readFileSync(join(fx.state, 'qa-calls.log'), 'utf8').trim().split(/\r?\n/).filter(p => p === fx.proj);
    assert.equal(calls.length, 1, 'landing 통합 QA를 재실행했다');
    assert.doesNotMatch(r.out, /\[INTEGRATION\]\[RETRY\]/);
  })
  it('[#16] 배치 매니페스트에 rollback 이 남고 push=false', () => {
    const m = batchManifest(fx)
    assert.ok(m, '배치 매니페스트 없음')
    assert.equal(m.schema, 'batch-24-multiag/batch-manifest/1')
  ...
  it('[#16/N6] rollback 이 각 스토리 매니페스트에 남는다 — 증거 사본 + batchId 붙은 미추적 sidecar', () => {
    ...
    for (const s of ['2-1-a', '2-2-b']) {
      ...
      assert.equal(v.schema, 'batch-24-multiag/verification/1', '엔진이 남긴 기존 필드 보존')
...
describe('[e2e][#7 대조군] 통합 GREEN 이면 실제로 원격에 push 된다(= 위 테스트의 「0건」이 헛것이 아니다)', { timeout: 300_000 }, () => {
  ...
  it('[#16] 각 스토리 검증 매니페스트에 integration=pass 가 병합되고, 작업 트리는 깨끗하게 남는다', () => {
    for (const s of ['2-1-a', '2-2-b']) {
      ...
      assert.equal(v.schema, 'batch-24-multiag/verification/1', '기존 필드 보존')
...
const seqFixture = () => makeFixture({
  models: { dev: 'fable', review: 'codex' },
  config: { providers: { codex: { enabled: true } }, integrationGate: { enabled: true } },
...
describe('[e2e][N1] 순차 + 통합 게이트 RED — 원격 ref 불변 · landing 되돌림 · 엔진에 --push 를 넘기지 않는다', { timeout: 300_000 }, () => {
  ...
  it('원격에 auto/* 가 하나도 없다 · 로컬 landing 0건 · exit 1', () => {
    ...
    assert.equal(commitsOnBranch(fx.proj).length, 0, 'landing 커밋도 되돌아가 있어야 한다')
    const calls = readFileSync(join(fx.state, 'qa-calls.log'), 'utf8').trim().split(/\r?\n/).filter(p => p === fx.proj);
    // Sequential workers share the main cwd: two worker typechecks plus one landing integration.
    assert.equal(calls.length, 3, '두 worker 검사와 landing 통합 검사 1회여야 한다');
    assert.equal((r.out.match(/\[INTEGRATION\]\[RUN\]/g) ?? []).length, 1, 'landing 통합 게이트는 1회');
    assert.doesNotMatch(r.out, /\[INTEGRATION\]\[RETRY\]/);
  })
...
describe('[e2e][N3] qa 가 흘린 시크릿이 통합 로그·증거·요약 어디에도 원문으로 남지 않는다', { timeout: 300_000 }, () => {
  ...
  let fx, r
  before(() => {
    fx = makeFixture({ models: { dev: 'fable', review: 'codex' }, config: { providers: { codex: { enabled: true } }, integrationGate: { enabled: true } } })
    fixtures.push(fx)
...
describe('[e2e] Codex 한도에도 자체 리뷰·완료·push 금지', { timeout: 300_000 }, () => {
  it('독립 리뷰 제공자가 한도이면 산출물을 보존하고 landing과 push를 차단한다', () => {
    const fx = makeFixture(); fixtures.push(fx);
    const r = runRunner(fx, { env: { E2E_CODEX_LIMIT: '1' } });
    assert.notEqual(r.status, 0, r.out);
    assert.doesNotMatch(r.calls, /^review 2-/m, 'Claude 자체 리뷰가 실행되면 안 된다');
    assert.equal(commitsOnBranch(fx.proj).length, 0);
    assert.deepEqual(originHeads(fx.proj), ['refs/heads/main']);
    assert.ok(evidenceDir(fx, '2-1-a'), '구현 산출물 복구 증거가 있어야 한다');
  });
});
...
describe('[e2e][OPS] 지출 한도 차단 — 실제 러너 경로에서 연속 카운트가 오르고 알림 판정이 쓰인다', { timeout: 300_000 }, () => {
  const exit5Fixture = (seed) => {
    const fx = makeFixture({ models: { dev: 'fable', review: 'codex' }, queueDefaults: { parallel: 1 } })
    fixtures.push(fx)
    // 엔진을 「한도(exit 5)로 아무것도 못 했다」 스텁으로 교체 — 실 LLM·실 배치 없이 그 갈래만 재현한다
    writeFileSync(join(fx.proj, 'tools', 'auto', 'runtime', 'auto-story-pipeline.mjs'), 'process.exit(5)\n')
    ok(git(fx.proj, ['add', '-A']), 'stub add'); ok(git(fx.proj, ['commit', '-qm', 'exit5 fixture']), 'stub commit');
...
describe('[e2e] Claude 전용(설정 없음) — 종전 명령줄 그대로 · codex 감지 0 · 통합 게이트 없음', { timeout: 300_000 }, () => {
  it('구 설정 키가 없어도 교차 리뷰와 필수 통합 게이트를 실행한다', () => {
    ...
    assert.equal(r.status, 0, r.out.slice(-2000))
    assert.match(r.out, /CODEX|codex/)
    assert.match(r.calls, /codex read-only/); assert.doesNotMatch(r.calls, /^review 2-/m)
    assert.match(r.summary, /\[INTEGRATION\]\[PASS\]/)
    assert.ok(!/워커 풀/.test(r.summary))
    assert.equal(commitsOnBranch(fx.proj).length, 3)
  })
})
...
describe('[e2e][#20] Claude-only 회귀 — 종전 큐·로그·exit 가 바이트 단위로 같다', { timeout: 300_000 }, () => {
  it('--queue 수동 큐: 구 큐도 독립 리뷰와 통합 게이트 뒤에만 landing한다', () => {
    const fx = claudeOnlyFixture()
    const r = runRunner(fx)
    assert.equal(r.status, 0, r.out.slice(-2000))
    assert.match(r.summary, /완주: 2-1-a/); assert.match(r.summary, /완주: 2-2-b/); assert.match(r.summary, /\[INTEGRATION\]\[PASS\]/)
    // 두 워커가 동시에 돌아 **기록 순서**는 경합한다 — 순서가 아니라 「무엇이 몇 번 불렸나」를 문다.
    assert.deepEqual(r.calls.split('\n').filter(Boolean).sort(), ['codex read-only 2-1-a', 'codex read-only 2-2-b', 'dev 2-1-a', 'dev 2-2-b', 'probe', 'probe'])
    assert.equal(commitsOnBranch(fx.proj).length, 3)
    assert.match(r.out, /CODEX|codex/)
  ...
  it('계측 파일은 남기되(덧붙임) 종전 절은 건드리지 않는다', () => {
    ...
    assert.equal(one.schema, 'batch-24-multiag/metrics/1')
...
describe('[e2e][#19] 마이그레이션 신규 2건 → 병렬 금지·순차 폴백', { timeout: 300_000 }, () => {
  it('겹침이 없어도 [PARALLEL][HAZARD] 로 순차 폴백하고 워크트리를 만들지 않는다', () => {
    ...
    assert.match(r.calls, /dev 2-1-a/)
    assert.equal(commitsOnBranch(fx.proj).length, 0, 'DB 검증 어댑터가 없는 변경은 완료·커밋 차단')
  })
...
describe('[e2e][#18] Fable 계획 — 채택 · 거부 폴백 · 기본 꺼짐', { timeout: 300_000 }, () => {
  ...
  it('지어낸 스토리·비JSON 은 거부하고 규칙 큐로 되돌아간다(밤이 서지 않는다)', () => {
    for (const [why, body] of [['invented-story', JSON.stringify({ batches: [{ stories: ['9-9-없는스토리'], stages: ['create', 'dev', 'review'] }] })], ['parse', '이건 JSON 이 아니다']]) {
      const fx = orchFixture(true)
      ...
      assert.ok(r.summary.includes('- 계획 출처: `deterministic-fallback(' + (why === 'invented-story' ? 'contract:' : '') + why), `${why}: ${r.summary}`)
    }
  })
...
const PLAN_BODY = JSON.stringify({
  rationale: '스텁 계획', batches: [
    { label: '스텁 A', stories: ['2-1-a'], stages: ['create', 'dev', 'review'] },
    { label: '스텁 B', stories: ['2-2-b'], stages: ['create', 'dev', 'review'] },
  ],
})
...
describe('[e2e][계획 캐시] 같은 후보면 다시 묻지 않는다 · 바뀌면 다시 묻는다', { timeout: 600_000 }, () => {
  ...
  it('캐시된 계획이 지금 규칙을 통과하지 못하면 버리고 다시 묻는다', () => {
    ...
    const r2 = runOrch(fx, stub)
    assert.equal(r2.status, 0, r2.out.slice(-3000))
    assert.match(r2.out, /캐시 계획이 지금 규칙을 통과하지 못한다\(deterministic-fallback\(contract:invented-story/, r2.out.slice(-2000))
  ...
  it('설치 템플릿의 orchestrator 기본값은 켜짐 + 캐시 12시간이다 (👤 2026-09-03 「(가)」)', () => {
    const src = readFileSync(join(REPO, 'batch-24-multiAG', 'install.mjs'), 'utf8')
    assert.match(src, /orchestrator: \{ enabled: true, model: 'fable', timeoutMin: 5, cacheHours: 12 \}/, '설치 기본값이 꺼짐으로 되돌아갔다')
    assert.match(src, /orchestrator/, '기본값을 켠 근거(👤 결정) 인용이 없다')
  })
})
...
describe('[e2e][assign] 배정 기록 — 라운드 뒤 갱신 · 연속 실패 회피', { timeout: 300_000 }, () => {
  it('라운드가 끝나면 스토리·프로바이더·역할별 기록이 상태 폴더에 남는다', () => {
    ...
    assert.equal(h.entries['2-2-b|codex|review'].attempts, 1, Object.keys(h.entries).join(','))
    assert.ok(!h.entries['2-2-b|claude|review'], 'max는 동시성 상한이며 자체 리뷰를 허용하지 않는다')
  })

  it('codex review 가 그 스토리에서 연속 2회 실패한 기록이어도 같은 제공자 자체 리뷰로 바꾸지 않는다', () => {
    const fx = makeFixture(); fixtures.push(fx)
    ...
    const r = runRunner(fx)
    assert.equal(r.status, 0, r.out.slice(-2000))
    assert.match(readFileSync(join(fx.art, 'auto-pipeline-logs/run-summary.log'), 'utf8'), /\[CODEX\]\[REVIEW\]/, '실제 리뷰 제공자가 Codex여야 한다')
    assert.match(r.calls, /codex read-only/); assert.doesNotMatch(r.calls, /^review 2-/m)
  })
})
...
describe('[e2e][M5] 통합 게이트 명령의 셸 결합 제거 — 두 번째 명령이 돌지 않고 push 도 없다', { timeout: 300_000 }, () => {
  ...
  before(() => {
    fx = makeFixture({
      models: { dev: 'fable', review: 'codex' },
      config: { providers: { codex: { enabled: true } }, integrationGate: { enabled: true }, qa: 'node tools/qa.mjs && node tools/evil.mjs' },
...
describe('[e2e] review 전용 배치 병렬 — File List 겹침 무시 · 장부 union landing · 통합 게이트 PASS', { timeout: 300_000 }, () => {
  ...
  it('review 전용 병렬에서도 구현자·품질 근거 없이 landing할 수 없다', () => {
    assert.equal(r.status, 1, r.out.slice(-3000))
    ...
    assert.doesNotMatch(r.summary, /완주: 2-1-a/)
    assert.doesNotMatch(r.summary, /완주: 2-2-b/)
    assert.equal(commitsOnBranch(fx.proj).length, 0)
  })

  it('검증되지 않은 두 스토리는 review 상태를 유지하고 코드도 만들지 않는다', () => {
    const sprint = readFileSync(join(fx.art, 'sprint-status.yaml'), 'utf8')
    assert.match(sprint, /^  2-1-a: review$/m, sprint)
    assert.match(sprint, /^  2-2-b: review$/m, sprint)
    for (const k of ['2-1-a', '2-2-b']) assert.match(readFileSync(join(fx.art, `${k}.md`), 'utf8'), /^Status: review$/m)
    assert.ok(!existsSync(join(fx.proj, 'src', 'a.ts')), '리뷰 전용 배치가 코드를 만들면 안 된다')
    assert.doesNotMatch(r.calls, /^review 2-/m, '자체 리뷰 실행 차단')
    assert.doesNotMatch(r.calls, /dev 2-/, 'dev 가 돌면 리뷰 전용이 아니다')
  })

  it('검증되지 않은 review-only 배치는 원격 ref와 로컬 커밋을 남기지 않는다', () => {
    assert.deepEqual(originHeads(fx.proj), ['refs/heads/main']);
    assert.equal(commitsOnBranch(fx.proj).length, 0);
  })
})
