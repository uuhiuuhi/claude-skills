...
export function assignWorkers({ stories = [], roles = ['dev', 'review'], providers = {}, history = null, config = {} } = {}) {
  if (config.modelPolicy?.enabled) return assignPolicyWorkers({ stories, roles, providers, history, config })
  ...
  return stories.map((s, i) => {
    ...
    if (wantDev) {
      ...
      // max is a concurrency cap; pickRunnable/withCodexSlot own enforcement.
...
export function assignPolicyWorkers({ stories = [], roles = ['dev', 'review'], providers = {}, history = null, config = {} }) {
  const wantDev = roles.includes('dev') || roles.includes('repair')
  const wantReview = roles.includes('review')
  const exhausted = new Set(config.exhaustedModels ?? [])
  return stories.map((s) => {
    const story = String(s.key ?? s.story ?? '')
    const scoredRisk = storyRisk(s), scoredDifficulty = storyDifficulty(s)
    const unknown = !String(s.text ?? '').trim() || !(s.files ?? []).length
    const risk = unknown ? Math.max(4, scoredRisk.score) : scoredRisk.score
    ...
    return { story, dev: dev?.model ?? '', review: review?.model ?? '',
      devProvider: dev?.provider ?? 'claude', reviewProvider: review?.provider ?? 'claude', risk, difficulty,
      flags: [...scoredRisk.flags, ...(unknown ? ['unknown-scope'] : [])],
      why: 'six-model-policy; unavailable roles are deferred by the engine' }
  })
}
