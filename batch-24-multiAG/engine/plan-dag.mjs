...
export function validatePlan(plan, dag = { nodes: [], edges: [], cycles: [], byKey: new Map() }, constraints = {}) {
  ...
  batches.forEach((b, bi) => {
    ...
    const writesCode = !Array.isArray(b?.stages) || b.stages.some((stage) => stage !== 'review')
  ...
  for (const [key, bi] of seen) {
    ...
    for (const d of deps) {
      ...
      if (target) {
        ...
        else if (tb >= bi) errors.push(E('dep-order', key, `선행 ${d} 이 같은/뒤 배치(${tb + 1})에 있다 — 선행 완료 후 실행해야 한다`))
