...
import { FIXTURE_HELPER } from './strict-quality-fixture.mjs';
...
async function devStory(cwd, key) {
  ...
  for (const rel of declaredFiles(md)) {
    ...
    if (!existsSync(p)) writeFileSync(p, `export const story_${key.replace(/[^A-Za-z0-9]/g, '_')} = 1\n`)
  }
  const helperPath = join(cwd, 'tools/quality-fixture.mjs');
  if (existsSync(helperPath)) { const { writeStoryTests } = await import(pathToFileURL(helperPath).href); writeStoryTests(cwd, key, declaredFiles(md)); }
