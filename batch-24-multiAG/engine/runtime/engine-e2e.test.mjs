...
if (m) {
  ...
  devWork(key)
  if (action === 'auth-code') writeStoryTests(cwd, key + '-auth', ['src/auth/session.ts']);
  console.log('dev 완료')
  process.exit(0)
}
...
if (m) {
  ...
  if (process.env.E2E_CLAUDE_TRACE === '1') {
    ...
    const paths = JSON.parse(line.slice('REVIEW_INPUTS_JSON:'.length))
    paths.forEach((p,i) => {
      readFileSync(join(cwd,p),'utf8')
      console.log(JSON.stringify({type:'assistant',message:{content:[{type:'tool_use',id:'read'+i,name:'Read',input:{file_path:join(cwd,p)}}]}}))
      console.log(JSON.stringify({type:'user',message:{content:[{type:'tool_result',tool_use_id:'read'+i,content:'read completed'}]}}))
    })
  }
...
if (sandbox === 'workspace-write') {
  ...
  const { writeStoryTests } = await import(pathToFileURL(process.cwd() + '/tools/quality-fixture.mjs').href)
  writeStoryTests(process.cwd(), story, ['src/a.ts'])
...
describe('[engine-e2e][#6] `--commit` 은 auto/* 또는 detached 에서만', { timeout: 180_000 }, () => {
  ...
  it('--branch auto/x → 브랜치 생성 후 스토리 커밋 1건', () => {
    const fx = makeFixture()
    const r = runEngine(fx, { args: ['--stages', 'dev,review', '--commit', '--branch', 'auto/2026-09-02'] })
    assert.equal(r.status, 0, r.out.slice(-2000))
  ...
  it('detached 워크트리(러너 landing 모드) → 종전대로 커밋된다', () => {
    ...
    const r = runEngine(fx, { args: ['--stages', 'dev,review', '--commit'], cwd: wt })
    assert.equal(r.status, 0, r.out.slice(-2000))
...
describe('[engine-e2e][#10] Claude 불가·Codex 정상 — 동일 제공자 자체 리뷰 차단', { timeout: 180_000 }, () => {
  it('Codex dev 는 Claude 프로브 없이 실행되지만 Codex 자체 리뷰는 차단한다', () => {
    const fx = makeFixture()
    const r = runEngine(fx, {
      args: ['--stages', 'dev,review', '--dev-model', 'codex:dev-m', '--review-model', 'codex:review-m'],
      env: { E2E_CLAUDE_401: '1' },
    })
    assert.notEqual(r.status, 0, r.out.slice(-3000))
    ...
    assert.ok(!r.calls.includes('codex read-only'), '자체 리뷰를 실행했다')
    assert.match(r.out, /review 모델 교차검증 조정/)
    assert.match(r.out, /\[CLAUDE\]\[REVIEW\]/)
    assert.notEqual(r.manifest('2-1-a')?.completion?.verdict, 'ready')
    assert.equal(commitsAhead(fx.proj).length, 0)
  })
})

describe('[engine-e2e] Codex implementation with independently observed Claude review', {timeout:180000}, () => {
  it('actual source reads plus QA authorize completion; the configured Codex self-review is replaced', () => {
    const fx=makeFixture(); const r=runEngine(fx,{args:['--stages','dev,review','--dev-model','codex:dev-m','--review-model','codex:review-m'],env:{E2E_CLAUDE_TRACE:'1'}});
    assert.equal(r.status,0,r.out);const m=r.manifest('2-1-a');assert.equal(m.completion.verdict,'ready');assert.equal(m.review.provider,'claude');assert.ok(m.review.readEvidence.length>=3);assert.ok(!r.calls.includes('codex read-only'));
  });
});
...
describe('[engine-e2e][#11] 미추적 신규 테스트 파일의 .only/skip/항상-참 단언이 잡힌다', { timeout: 180_000 }, () => {
  it('`tests/new.test.ts` 를 새로 만들며 describe.only 를 넣으면 무결성 차단 STOP', () => {
    ...
    assert.match(r.out, /\[INTEGRITY\]\[BLOCK\] test-skip tests\/new\.test\.ts/)
    assert.match(r.out, /\[INTEGRITY\]\[BLOCK\] trivial-assertion tests\/new\.test\.ts/)
    assert.match(r.out, /테스트 무결성 차단 3건\(test-only, test-skip, trivial-assertion\)/)
  })
})
...
describe('[engine-e2e][#14] 트리거된 보안 게이트를 실제로 실행하고 실패는 RED 로 전파한다', { timeout: 180_000 }, () => {
  ...
  it('필수 security 스크립트가 없으면 required-missing 기록과 함께 완료를 차단한다', () => {
    const fx = makeFixture()
...
describe('[engine-e2e][검증표#2] 빈 diff 로 인한 codex→claude 전환도 전환 횟수에 든다', { timeout: 180_000 }, () => {
  it('볼 diff 가 없으면 claude 리뷰로 넘기고, 그 전환을 스토리당 상한 장부에 기록한다', () => {
    ...
    assert.ok(!/^review 2-1-a$/m.test(r.calls), '같은 제공자 fallback 리뷰가 실행됐다')
    assert.equal(commitsAhead(fx.proj).length, 0)
  })
})
...
describe('[engine-e2e][N1] `--push` + `--e2e` — 배치 e2e RED 면 원격 ref 가 움직이지 않는다', { timeout: 180_000 }, () => {
  it('e2e RED → exit 1 · push 0회(원격 불변) · 커밋은 로컬 auto/* 에 남는다', () => {
    ...
    const before = originHeads(fx.proj)
    const r = runEngine(fx, {
      args: ['--stages', 'dev,review', '--commit', '--branch', 'auto/2026-09-02', '--push', '--e2e', 'node tools/batch-e2e.mjs'],
  ...
  it('`--e2e` 가 없으면 종전대로 스토리 단위로 push 한다(하위 호환)', () => {
    const fx = makeFixture()
    const r = runEngine(fx, { args: ['--stages', 'dev,review', '--commit', '--branch', 'auto/2026-09-02', '--push'] })
    assert.equal(r.status, 0, r.out.slice(-2000))
...
describe('[engine-e2e][push-guard] `--push --branch main` 은 시작조차 못 한다', { timeout: 180_000 }, () => {
  ...
  it('`--push --branch master`·`--branch release` 도 같다(보호 이름 전반)', () => {
    for (const b of ['master', 'release', 'production']) {
      const fx = makeFixture()
      const before = originHeads(fx.proj)
      const r = runEngine(fx, { args: ['--stages', 'dev,review', '--commit', '--branch', b, '--push'] })
  ...
  it('대조군 — `--branch auto/2026-09-03` 은 종전대로 1회 push 한다(자기 RED)', () => {
    const fx = makeFixture()
    const r = runEngine(fx, { args: ['--stages', 'dev,review', '--commit', '--branch', 'auto/2026-09-03', '--push'] })
    assert.equal(r.status, 0, r.out.slice(-2000))
...
describe('[engine-e2e][autonomy] replan 단계 — 결정 채택·회수 Task 개설·인박스 사후 확인', { timeout: 180_000 }, () => {
  it('replan→dev→review 가 한 배치로 돌고 열린 Decision 이 AI 결정으로 닫힌다', () => {
    ...
    assert.equal(r.status, 0, r.out.slice(-2000))
    assert.match(r.calls, /replan 2-1-a[\s\S]*dev 2-1-a[\s\S]*codex read-only 2-1-a/, r.calls)
...
describe('[engine-e2e][autonomy] mockup 단계 — AI 초안 + 장부 pending', { timeout: 180_000 }, () => {
  it('장부에 pending 항목이 생기고, 커밋 화이트리스트에 목업 폴더를 주면 HTML 도 같은 커밋에 실린다', () => {
    const fx = makeFixture()
    const r = runEngine(fx, { args: ['--stages', 'mockup,dev,review', '--autonomy', 'full', '--commit', '--branch', 'auto/2026-09-03', '--commit-paths', 'src,tests,tools,_bmad-output,mockups'] })
    assert.equal(r.status, 0, r.out.slice(-2000))
