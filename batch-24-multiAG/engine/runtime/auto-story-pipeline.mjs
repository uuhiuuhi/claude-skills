...
const KIND = {
  ...
  spend: {
    ...
    fix: "기다려도 풀리지 않습니다. 사람이 claude.ai/settings/usage 에서 「사용 크레딧」을 켜거나 지출 한도를 올려야 합니다(👤 정책: 기본은 구독 한도까지만 · 크레딧 사용은 승인자 판단). 모델을 바꿔도 소용없습니다 — 계정 전체 지갑입니다",
    exit: 5,
  },
};
...
const prompts = {
  ...
  replan: (s) => [
    ...
    "3) 열린 `- [ ] [Review][Patch]` 가 있는데 Tasks/Subtasks 절에 미완 기계 Task 가 없으면, Tasks 절 끝에 `### 회수 라운드 <날짜>` 소제목과 각 Patch 를 고치는 구체적 `- [ ] ` Task 를 적는다(「사람 게이트」·「승인자」·👤 표기는 쓰지 않는다 — 그 표기는 사람만 풀 수 있다는 뜻이다).",
...
function applyCodexReview(story, res, w) {
  ...
  const readEvidence = [...new Set([ri.storyFile, ri.diffFile, ...(ri.changedFiles ?? [])].filter(Boolean))]
    .filter((path) => readEvidenceFor(path, { commands: res.events?.commandList ?? [], filePaths: res.events?.filePaths ?? [] }));
  reviewResults[story] = { provider: "codex", model: w.spec.model || "default", result: newStatus === "done" ? "clean" : "findings", counts: r.counts, status: newStatus, warnings: v.warnings, readEvidence };
...
function prepareWorker(stage, story, variant) {
  ...
  const resolved = routingEnabled ? { spec: parseModelSpec(models[stage]), fallback: false }
    : resolveWorkerSpec({ spec: models[stage], availability: wantsCodex ? providerAvailability() : {}, avoid, codexCwd: wantsCodex ? codexCwdInfo() : { ok: true } });
  ...
  if (spec.provider === "codex") {
    ...
    const effort = routingEnabled ? MODEL_CATALOG[formatModelSpec(spec)]?.effort : null;
...
function runClaude(stage, story, variant = null) {
  ...
  {
    const kind = routingEnabled ? (timedOut ? 'transient' : failureKind(w.provider === 'codex' ? codexFailureText(res) : combined)) : w.classify(combined, res);
    if (routingEnabled) lastRoutingFailure = { detail: redactSecrets(w.provider === 'codex' ? codexFailureText(res) : combined).slice(-1500) };
...
function runStage(stage, story, variant = null) {
  if (routingEnabled) return runRoutedStage(stage, story, variant);
...
function knownDevModel(story) {
  const worker = state.workers?.[key(story, 'dev')];
  if (worker?.model) return worker.model;
  // Legacy provenance may guide reviewer selection, but is never promoted to verified evidence.
  try {
    const old = JSON.parse(readFileSync(resolve(logDir, `${story}-verification.json`), 'utf8')).workers?.dev;
    if (old?.model) return old.provider === 'codex' ? `codex:${old.model}` : old.model;
  } catch { /* no previous implementation record */ }
  return '';
}
function runRoutedStage(stage, story, variant) {
  const profile = routedProfile(story);
  const availability = dryRun ? {} : providerAvailability();
  const providers = Object.fromEntries(['claude', 'codex'].map((p) => [p, {
    ...routingConfig.providers?.[p],
    available: dryRun ? true : availability[p]?.available === true,
  }]));
  ...
  for (;;) {
    ...
    const previous = attempted.at(-1);
    if (previous && providerOf(previous) !== selected.provider) {
      if ((switchesUsed[story] ?? 0) >= 1) {
        attempted.push(selected.model);
        continue;
      }
      countProviderSwitch(story, stage, selected.model, '공유 모델 상태·품질 기준');
    }
    models[stage] = selected.model;
    note(`[MODEL-ROUTE] story=${story} stage=${stage} model=${selected.model} risk=${profile.risk} difficulty=${profile.difficulty}${previous ? ` from=${previous}` : ''}`);
    lastRoutingFailure = {};
    const requestStarted = Date.now();
    const result = runClaude(stage, story, variant);
    if (result === 'ok') {
      if (!dryRun) {
        router.record(selected.model, 'success', { role: stage, story, startedAt: requestStarted });
        state.workers ??= {};
        state.workers[key(story, stage)] = { model: selected.model, provider: selected.provider, at: stamp() };
        if (stage === 'review' && reviewResults[story]) {
          state.reviews ??= {};
    ...
    const workers = {};
    {
      for (const st of KNOWN_STAGES) {
        ...
        if (actual && isDone(story, st)) {
          const spec = parseModelSpec(actual.model);
          workers[st] = { provider: spec.provider, model: spec.model, at: actual.at, provenance: 'successful-worker' };
        }
      }
    }
...
for (const story of stories) {
  ...
  if (routingEnabled) {
    note(`↷ [${story}] 별도 유료 프로브 생략 — 실제 다음 단계가 공유 상태를 확인하고 한도·인증을 판정한다`);
  } else if (!dryRun && storyNeedsWork(story) && !storyNeedsClaude(story)) {
