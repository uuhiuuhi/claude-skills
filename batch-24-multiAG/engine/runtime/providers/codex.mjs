...
export function buildCodexCommand({ bin = 'codex', role = 'review', cwd, model = '', schemaPath = null, outFile = null, networkAccess = false, ephemeral = true, skipGitCheck = false, reasoningEffort = '', extraConfig = [] } = {}) {
  ...
  if (reasoningEffort) {
    ...
    if (!CODEX_REASONING_EFFORTS.includes(effort)) throw new UnsafeArgumentError('추론 강도', effort)
    // Codex config is TOML. String values require quotes; this value comes only
    // from the fixed allow-list above and argv is passed with shell:false.
