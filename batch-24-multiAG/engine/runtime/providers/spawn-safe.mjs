...
/** Codex의 TOML 문자열 설정 중 엔진이 고정 목록에서 생성하는 추론 강도 한 종류만 허용한다. */
export const SAFE_CODEX_REASONING_CONFIG_RE = /^model_reasoning_effort="(?:none|minimal|low|medium|high|xhigh|max|ultra)"$/
...
export function planSpawn(file, argv = [], { platform = process.platform, comspec = process.env.ComSpec || 'cmd.exe', exec = spawnSync, env = process.env } = {}) {
  ...
  const args = argv.map((a, i) => {
    const s = String(a ?? '')
    // buildCodexCommand의 전용 allow-list 경로만 TOML 따옴표를 쓸 수 있다.
    // 일반 config/argv의 따옴표 허용 범위는 넓히지 않는다.
    if (argv[i - 1] === '-c' && SAFE_CODEX_REASONING_CONFIG_RE.test(s)) return s
    return assertSafe(s, `인자[${i}]`)
  })
