...
describe('[quality] 테스트 무결성 — 꼼수 탐지 · 정당한 삭제는 사유로 통과', () => {
  ...
  it('block: .only · 사유 없는 테스트 삭제 / warn: skip · ts-ignore · eslint-disable · 항상 참 단언 · 단언 약화 · exclude', () => {
    ...
    assert.ok(rules.includes('block:test-skip'))
    assert.ok(rules.includes('block:ts-ignore'))
    assert.ok(rules.includes('block:eslint-disable'))
    assert.ok(rules.includes('block:trivial-assertion'))
    assert.ok(rules.includes('block:assertion-weakened'))
    assert.ok(rules.includes('block:coverage-exclude'))
  ...
  it('스토리에 삭제 사유가 있으면 warn 으로 내려간다', () => {
    ...
    assert.deepEqual(f.map((x) => x.level + ':' + x.rule), ['block:deleted-test-justified'])
  })
...
describe('[quality] Node native coverage bypass', () => {
  it('coverage disabling and ignore-next comments are blocked; quoted documentation is allowed', () => {
    const scan = line => testIntegrityFindings({diff:'diff --git a/src/a.mjs b/src/a.mjs\n--- a/src/a.mjs\n+++ b/src/a.mjs\n@@ -0,0 +1 @@\n+'+line+'\n'});
    for (const directive of ['// node:coverage disable','/* node:coverage ignore next */']) assert.ok(scan(directive).some(f=>f.rule==='coverage-exclude' && f.level==='block'));
    assert.deepEqual(scan('const docs = "node:coverage disable";'),[]);
  });
});
