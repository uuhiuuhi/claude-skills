...
const policy = readFileSync(join(here, 'quality-gates.mjs'), 'utf8')
...
describe('[engine-guards] 현황판이 읽는 로그 줄 형식은 그대로다(dev-status scan.mjs reviewRuns/parseBatch)', () => {
  it('`→ [story] <stage> (model=…, perm=…)` 템플릿과 `   exit=<code>` 줄이 그대로 있다', () => {
    ...
    assert.ok(has('note(`→ [${story}] qa-gate: batch-24-multiag scoped quality`)'))
    assert.ok(has('note(`   qa exit=${policyRun.status} log=${policyFile}`)'))
  })
...
describe('[engine-guards] v3 다중 프로바이더 규율', () => {
  ...
  it('로그 마스킹은 프로바이더를 가리지 않는다(정책 2) — qa·워커·조건부 게이트 전부 scrubLog 경유', () => {
    ...
    assert.ok(has('scrubLog(q.policy.gates.filter'), 'qa 로그 마스킹')
  ...
  it('조건부 게이트는 있으면 실제로 실행하고 실패는 RED 다(#10) · 인박스는 없으면 만든다(#13)', () => {
    assert.ok(policy.includes('export async function executeGate('))
    assert.ok(has("join(SKILL_DIR, 'quality-gates.mjs')"))
    assert.ok(has("['security', 'performance'].includes(gate.name)"))
    assert.ok(has("if (gate.result === 'fail')"))
  ...
  it('자동 수리 기본 0(종전 동작) · 예산 판정은 repairDecision 하나 · 수리 라운드 신규 흔적 승격', () => {
    ...
    assert.ok(policy.includes('testIntegrityFindings('))
    assert.ok(has('blocks = q.integrity.filter'))
  ...
  it('[M5] 엔진에 `shell: true` 가 0건이다 — 자유형 명령은 planCommand 로 argv 분리', () => {
    ...
    assert.ok(has('if (opt("qa", "")) planCommand("qa 게이트(--qa)", qaCmd);'))
    assert.ok(has('const e2ePlan = planCommand("배치 e2e(--e2e)", e2eCmd);'))
    assert.ok(policy.includes('integrationGateInvocation(`npm run ${gate.script}`)'))
    assert.ok(policy.includes('shell: false'))
