 ...
 *
 * @param {{manifest:object, storyText?:string, diff?:string, testEvidence?:object|null, sprintStatus?:string|null}} o
 ...
 */
export function strengthenCompletion({ manifest, storyText = '', diff = '', testEvidence = null, sprintStatus = null } = {}) {
  const m = manifest ?? {}
  ...
  const tests = testEvidence && typeof testEvidence === 'object'
    ? {
        measurable: Boolean(testEvidence.measurable),
        files: arr(testEvidence.files).map(str),
        cases: Number.isFinite(Number(testEvidence.cases)) ? Number(testEvidence.cases) : 0,
