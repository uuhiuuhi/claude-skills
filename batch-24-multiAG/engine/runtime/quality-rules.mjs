...
const CONFIG_COVERAGE_RE = /(?:vitest|vite|nyc|c8)\.config\.[cm]?[jt]s$|jest\.config/
...
// Mask strings/comments without changing line positions. Fixture text is not executable test code.
export function maskJavaScript(text) {
  let code = '', comments = '', state = 'code', quote = '', escaped = false;
  for (let i = 0; i < text.length; i++) {
    ...
    if (state === 'string') {
      ...
      comments += c; code += c === '\n' ? '\n' : ' ';
      if (c === '*' && next === '/') { comments += next; code += ' '; i++; state = 'code'; }
    } else if (c === '"' || c === "'" || c === '`') { state = 'string'; quote = c; code += ' '; }
    else if (c === '/' && (next === '/' || next === '*')) { state = next === '/' ? 'line' : 'block'; comments += c + next; code += '  '; i++; }
    else code += c;
  }
...
export function testIntegrityFindings({ changes = [], diff = '', storyText = '' } = {}) {
  ...
  for (const c of changes) {
    ...
    if (st === 'D' && TEST_FILE_RE.test(p)) {
      if (deletedTestJustified(storyText, p)) push('block', 'deleted-test-justified', p, 0, '테스트 파일 삭제 — 스토리에 사유 기재됨')
  ...
  for (const [path, f] of Object.entries(files)) {
    ...
    const scan = maskJavaScript(f.added.map(a => a.text).join('\n'));
    const codeLines = scan.code.split('\n');
    for (const [index, a] of f.added.entries()) {
      const t = a.text, executable = codeLines[index]
      if (/\b(it|test|describe)\.only(?:\.each)?\s*\(/.test(executable)) push('block', 'test-only', path, a.line, '`.only` 는 나머지 테스트를 조용히 끈다 — 커밋 전 제거', t)
      if (/\b(it|test|describe)\.(?:skip|skipIf|runIf)\s*\(|\bx(it|test|describe)\s*\(|\btest\.todo\s*\(/.test(executable)) push('block', 'test-skip', path, a.line, '테스트 skip/todo 추가 — 사유가 스토리에 있어야 한다', t)
      if (/@ts-ignore/.test(t) && /@ts-ignore/.test(scan.comments)) push('block', 'ts-ignore', path, a.line, '@ts-ignore 추가 — 타입 오류 은폐 의심(@ts-expect-error + 사유 권장)', t)
      if (/eslint-disable/.test(t) && /eslint-disable/.test(scan.comments)) push('block', 'eslint-disable', path, a.line, 'eslint-disable 추가 — 규칙 비활성화 사유 필요', t)
      if (isTest && /expect\(\s*true\s*\)\.toBe\(\s*true\s*\)|expect\(\s*1\s*\)\.toBe\(\s*1\s*\)|assert\.ok\(\s*(?:1|true)\s*\)|assert\.(?:equal|strictEqual)\(\s*(true\s*,\s*true|1\s*,\s*1)\s*\)/.test(executable)) push('block', 'trivial-assertion', path, a.line, '항상 참인 단언 — 결함을 재현하지 못한다', t)
      if (CONFIG_COVERAGE_RE.test(path) && /exclude\s*:/.test(t)) push('block', 'coverage-exclude', path, a.line, '커버리지·테스트 exclude 추가 — 사유 필요', t)
      // 빈 테스트 본문 — `it('x', () => {})` 은 언제나 통과한다(단언 0). trivial assertion 과 같은 갈래.
      if (isTest && /\b(it|test)\s*\(\s*(['"`])[^'"`]*\2\s*,\s*(async\s*)?\(\s*\)\s*=>\s*\{\s*\}\s*\)/.test(t)) push('block', 'empty-test', path, a.line, '본문이 빈 테스트 — 단언 0건이라 언제나 통과한다', t)
    }
    ...
    if (isTest && !out.some(finding => finding.file === path && finding.rule === 'empty-test') && /\b(?:it|test)\s*\(\s*,\s*(?:async\s*)?\([^)]*\)\s*=>\s*\{\s*\}\s*\)/.test(scan.code)) push('block', 'empty-test', path, 0, 'empty multiline test');
    ...
    if (GATE_CONFIG_RE.test(path)) {
      for (const key of ['lines', 'branches', 'functions', 'statements']) {
        ...
        const old = re.exec(f.removed.map(a => a.text).join('\n')), next = re.exec(addedText);
        if (old && next && Number(next[1]) < Number(old[1])) push('block', 'coverage-threshold-lowered', path, 0, key + ' threshold decreased');
      }
    }
    if (isTest && f.removed.some(a => /to(?:Be|Equal|StrictEqual)\s*\(|assert\.(?:equal|strictEqual|deepEqual)/.test(a.text)) && /toBeTruthy\s*\(|toBeDefined\s*\(|assert\.ok\s*\(/.test(addedText)) push('block', 'assertion-weakened', path, 0, 'specific assertion replaced with weaker truthiness check');
    if (isTest) {
      const removedCases = f.removed.filter(a => /\b(?:it|test)\s*\(/.test(a.text)).length;
      const addedCases = f.added.filter(a => /\b(?:it|test)\s*\(/.test(a.text)).length;
      if (removedCases > addedCases) push('block', 'deleted-test-case', path, 0, 'test cases removed');
      const removedExpect = f.removed.filter((r) => /\b(?:expect\s*\(|assert(?:\.[\w]+)?\s*\()/.test(r.text)).length
      const addedExpect = f.added.filter((r) => /\b(?:expect\s*\(|assert(?:\.[\w]+)?\s*\()/.test(r.text)).length
      // 순 감소 2 이상 = 「단언 3개를 항상-참 1개로 바꾼」 류. 경고일 뿐이라(차단 아님) 오탐 비용은 기록 한 줄이다.
      if (removedExpect > addedExpect) push('block', 'assertion-weakened', path, 0, `단언 ${removedExpect}개 제거 · ${addedExpect}개 추가 — 단언 약화 의심`, `-${removedExpect}/+${addedExpect}`)
    }
  }
  return out
}
...
export const MANIFEST_SCHEMA = 'batch-24-multiag/verification/1'
