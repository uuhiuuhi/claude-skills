...
describe('[providers] 교차검증 — 같은 프로바이더·같은 모델이면 review 를 바꾼다', () => {
  it('claude 동일 모델 → 사다리에서 dev 와 다른 첫 모델(종전 enforceCrossModel 과 동일)', () => {
    assert.deepEqual(enforceCrossSpec({ dev: 'opus', review: 'opus' }), { review: { provider: 'codex', model: 'gpt-5.6-sol' }, changed: true })
    assert.deepEqual(enforceCrossSpec({ dev: 'fable', review: 'fable' }).review, { provider: 'codex', model: 'gpt-6-astra' })
    assert.equal(enforceCrossSpec({ dev: 'opus', review: 'fable' }).changed, true)
    assert.equal(enforceCrossSpec({ dev: '', review: '' }).changed, true) // 둘 다 cli-default 는 종전처럼 손대지 않는다(새 프로세스=새 컨텍스트)
  })
...
describe('[claude] 어댑터 — 인자 계약은 종전 그대로 · 우회 플래그 없음 · 셸 결합 없음(#6)', () => {
  ...
  it('env 를 주면 그대로 자식에 전달한다(git-guard shim PATH 배선 지점)', () => {
    ...
    assert.deepEqual(seen.env, { PATH: '/shim:/usr/bin', ...(process.env.NODE_V8_COVERAGE ? { NODE_V8_COVERAGE: process.env.NODE_V8_COVERAGE } : {}) })
  })
})
