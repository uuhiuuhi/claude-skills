#!/usr/bin/env node
// batch-24-multiAG 설치기 — **대상 프로젝트 루트에서** 실행한다.
//
...
if (existsSync(routingGuide) && !has('force')) notes.push('· MODEL-ROUTING.md 이미 있음 — 건너뜀(덮어쓰려면 --force)')
  ...
  copyFileSync(join(SELF, 'references', 'MODEL-ROUTING.md'), routingGuide)
  console.log('✔ tools/auto/MODEL-ROUTING.md')
}
...
if (!existsSync(cfgPath)) {
  writeFileSync(cfgPath, JSON.stringify({
    _readme: [
      ...
      'workers: { max: 총 동시 워커(기본 3 = 종전 하드캡 · 절대 상한 6), batchSize: 한 번에 편성할 후보 수(기본 3) }',
...
if (clonePath) {
  ...
  writeFileSync(join(clonePath, '.auto-batch-worktree'), 'batch-24-multiAG 실행 전용 클론 marker — 러너가 이 파일이 있을 때만 checkout -f 새로고침을 한다\n', 'utf8')
...
const engineFiles = [
  ...
  'auto.config.json',
  'runtime/auto-story-pipeline.mjs',
  'runtime/providers/index.mjs',
  'MODEL-ROUTING.md',
]
...
const taskXml = `<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Description>${esc(`batch-24-multiAG 무정지 러너 — 00:05 시작 · 30분 간격 무기한 · ${runDir} 에서 --auto-plan`)}</Description>
  </RegistrationInfo>
  <Principals>
    <Principal id="Author">
