...
Claude Code와 Codex에서 함께 쓸 수 있는 자동화 스킬 모음이다.

| 스킬 | 하는 일 |
|---|---|
| [`batch-24-multiAG`](batch-24-multiAG/) | BMad 24시간 배치에서 Claude Fable·Opus·Sonnet 구현과 Codex Astra·Sol·Terra 독립 리뷰를 등급별로 묶어 운용한다. Fable 한도 시 Opus 전환, 공유 모델 상태, 병렬 워커, QA·지문 검증, 프로젝트 고정 런타임을 포함한다. |
...
- **여섯 모델 배치**: `batch-24-multiAG`를 Claude와 Codex 전역 스킬 폴더에 복사하고 대상
  프로젝트 루트에서 `node <클론>/batch-24-multiAG/install.mjs --force`를 실행한다. 실행 중인
  운영 러너는 건드리지 않고 lock과 PID가 모두 사라진 라운드 경계에서만 교체한다.

모든 스킬은 Node 20 이상을 사용한다. 외부 npm 의존성은 없다.
