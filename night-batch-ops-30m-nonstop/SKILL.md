# night-batch-ops (30분 슬롯 무정지판) — 24시간 무인 배치 러너 · **현행 정본**

> **이 판은 무정지(non-stop) 방식이다.** 시계가 **30분마다 한 번씩 두드리기만** 하고, 겹침·미머지·
> 충돌·중단 판정은 전부 러너가 스스로 한다. 목표는 하나다 — **아침 브리핑부터 다음 아침까지
> 러너가 서 있는 시간을 없애되, 완성도 게이트는 한 칸도 낮추지 않는다.**
> 저빈도 운영이나 롤백이 필요하면 4시간 슬롯 보존판(`night-batch-ops-4h-slots`)을 쓴다.
...
**계층 원칙**: 이 폴더의 엔진 5파일은 프로젝트 중립이다. 프로젝트 고유값(에픽 순서·하루 상한·
...
| `engine/runner-rules.mjs` | 순수 판정 규칙(테스트 가능 분리) — 차단기·병렬 조건·File List 겹침·무정지 판정부 |
...
| 항목 | 4시간 슬롯판(`night-batch-ops-4h-slots`) | 30분 무정지판(이 판) |
|---|---|---|
| 예약 작업 | 2개 — 18:00 데일리 + 4시간 슬롯 5회 | **1개** — 00:05 시작 · 30분 · 무기한 |
| 미머지 `auto/*` | 슬롯 **휴면**(사람 머지가 열쇠) | **선형 승계**로 계속(비정형 이름만 휴면) |
| `origin/main` 반영 | 다음 날 main 에서 새로 시작할 때만 | **라운드마다 하향 동기**(3분기 처분) |
...
| 하루 상한 연장 | 없음(자정까지 대기) | **`/extend N`**(당일 한정 · 코드 되묻기) |
| 신규 착수 제동 | 없음 | **체인 게이트 2일** |
| main 머지 · qa · 적대 리뷰 · 커밋 가드 | 사람 / 불변 | **동일**(무정지가 완화하지 않는다) |

**롤백** — 완성도 문제가 아니라 환경이 안 맞을 때의 되돌리기 경로다. `auto.config.json` 스키마와
큐 형식은 두 판이 같으므로 설정을 새로 만들 필요는 없다.

1. **예약만 되돌리기**(1차): `Disable-ScheduledTask -TaskName "<프로젝트>-nonstop"` →
   보관돼 있던 구판 두 작업을 `Enable-ScheduledTask -TaskName "<프로젝트>-night-batch"` ·
   `… "<프로젝트>-auto-slots"`. 무정지판 설치기는 **구판 작업을 삭제하지 않는다**(롤백 경로 보존).
2. **엔진까지 되돌리기**: 4시간 슬롯판 폴더에서 `node <4h폴더>/install.mjs --force` 로 `tools/auto/`
   의 엔진 파일을 덮어쓴다.
3. 남아 있는 무정지 상태 파일(`chain-info.json`·`cap-extend-*.json`)은 구판 엔진이 읽지 않으므로
   그대로 둬도 무해하다. `auto-plan-state.json` 의 `progressed`·`sigs` 필드도 구판은 무시한다.
