...
test('실물 BaroOS 저장소 읽기 전용 스모크(있을 때만)', { skip: existsSync(join(JNG, '_bmad-output')) ? false : 'jng-os 없음' }, () => {
  ...
  assert.ok(snap.stories.length >= 84, 'sprint 스토리 행 수(2026-09-02 실측 84) 이상(실물 저장소는 매일 자란다 — 하한만 문다)')
  // 실물 저장소는 매일 done 이 는다 — 정확값 대신 하한(2026-09-02 실측 19)으로 문다(브랜치·날짜에 흔들리지 않게)
  assert.ok(snap.stories.filter((s) => s.statusInSprint === 'done').length >= 19, '선언 done 19건 이상')
  assert.ok(snap.orphanStoryDocs.length >= 1, '고아 문서 1건(11-5 관리팀 질의서) 이상(실물 저장소는 매일 자란다 — 하한만 문다)')
  assert.match(snap.orphanStoryDocs[0].name, /^11-5-관리팀-질의서/)
  assert.ok(snap.epicStories.length >= 83, 'epics.md 스토리 절 83건 이상(실물 저장소는 매일 자란다 — 하한만 문다)')
  assert.ok(snap.code.srcCount >= 230, 'src 코드 파일 230 이상(실물 저장소는 매일 자란다 — 하한만 문다)')
