...
3. **qa 게이트·적대 리뷰·커밋 가드는 불변.** `npm run qa` 실패는 그대로 STOP 이고, 리뷰 단계도
   ...
   건드리지 않았다.
4. **main 머지는 사람이 한다.** (2026-09-03 👤 「무료 운영 안전장치 ②④」) 이건 규율이 아니라 **코드**다 — 러너의 push 는 `push-guard.mjs` 의 `safeGitPush()` 한 곳뿐이고(러너 안에 날 `git push` 0개), ref 가 `auto/*` 가 아니거나 현재 브랜치와 다르거나 미는 몫에 금지 경로·시크릿이 있으면 **PUSH GUARD STOP — push 0회**(로컬 커밋은 남는다 · 알림 발송). GitHub Free 는 비공개 저장소 `main` 을 서버가 못 막는다(2026-09-03 실측 · 룰셋 API 403) — 서버 측 보호(INSTALL-ISOLATION A+D)는 **팀 개발 단계 전환 작업**으로 미뤘다. 대화형 머지·배포 절차의 정본은 jng-os `tools/auto/merge-main.mjs`. 러너는 `auto/<날짜>` 브랜치까지만 커밋·푸시하고, main 방향은
