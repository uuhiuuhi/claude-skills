...
//
// 명령 등급(운영 승인 체계): 허용 = 재개·머지(읽기 /status 포함) / 불가 = 운영 DB 변경·
// 외부 발송·삭제·시크릿. 이 파일의 파서에는 허용 3개(/status /merge /resume)만 존재한다 —
// 「불가」 명령은 파서에 없음 = 입구 거부. 인자도 받지 않는다(임의 ref 머지 차단).

/** 인식하는 명령 전부. 여기 없는 텍스트는 무시(회신 없음). */
export const COMMANDS = Object.freeze(['/status', '/merge', '/resume'])
...
export const NEEDS_CONFIRM = Object.freeze(['/merge', '/resume'])

/** 명령 파서 — 정확히 명령 하나만(`/merge@botname` 접미사 허용). 인자가 붙으면 null(AC-4). */
...
/** 머지 대상 refspec — 원격 ff 전용. 로컬 체크아웃·작업 트리 접촉 0(AC-2).
...
/** 슬롯 재개 상태 수술(순수) — 이 창의 stops 만 0 으로. 다른 창·날짜 원장은 불변. */
export function resetWindowStops(state, windowId) {
  const s = structuredClone(state ?? {})
  s.windows ??= {}
  s.windows[windowId] = { ...(s.windows[windowId] ?? {}), stops: 0 }
  return s
}
