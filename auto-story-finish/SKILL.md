...
- **모델 품질 사다리 + 교차검증 강제.** ① 특정 모델**만** 한도(월 지출 한도 포함 — "monthly spend limit" 문구도 LIMIT 로 인식)에 걸리면 배치를 세우지 않고 한 단계 아래 모델로 자동 강등해 계속한다(사다리 예: fable→opus→sonnet, 대기 없음·알림 1회). 강등은 단계 실행(runStage)과 스토리 경계 프로브 **양쪽**에 적용된다 — 프로브만 옛 모델로 무한 대기하는 사각 봉합. ② review 모델이 dev 가 실제로 쓴 모델과 같아지면(강등 결과 포함) 리뷰를 다른 모델로 비켜 세운다 — **같은 LLM 의 자기 리뷰 방지**(교차검증). ③ 경계 프로브의 기본 모델은 CLI 기본값이 아니라 **배치 dev 모델**(`effectiveProbeModel`) — 배치와 무관한 모델의 한도로 오판하지 않게.
...
- `--e2e "<명령>"` = **배치 종료(전 스토리 완주) 후 1회** e2e 스모크 — RED면 exit 1(스토리 산출물은 유지, 커밋 전 확인 신호). 프로젝트가 e2e 명령을 보유·GREEN 확인된 경우에만 지정(예: `cd apps/web && npm run e2e:smoke` 형태 — 로컬 빌드+서버 기동 포함, 수 분 소요). **스토리마다 돌리지 않는다**(비용) — 신규 spec 작성도 이 스킬 범위 밖.
